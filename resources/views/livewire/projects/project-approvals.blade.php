    ...
    <div class="container mx-auto px-4 py-8">
        <div class="max-w-6xl mx-auto">
            ...
            <div class="bg-white rounded-lg shadow-md p-4 mb-6">
                <h3 class="text-lg font-semibold mb-3">Project Summary</h3>
                <div class="grid grid-cols-1 md:grid-cols-3 gap-3">
                    <div>
                        <p class="text-xs text-gray-600">Start Date</p>
                        <p class="text-sm font-semibold">{{ $project->start_date->format('M d, Y') }}</p>
                    </div>
                    <div>
                        <p class="text-xs text-gray-600">Client</p>
                        <p class="text-sm font-semibold">{{ $project->client?->name ?? 'N/A' }}</p>
                    </div>
                    <div>
                        <p class="text-xs text-gray-600">Total Budget</p>
                        <p class="text-sm font-semibold text-blue-600">${{ number_format($totalBudget, 2) }}</p>
                    </div>
                </div>
                ...
                @if($project->description)
                    <div class="mt-3">
                        ...
                        <p class="text-xs text-gray-800">{{ $project->description }}</p>
                    </div>
                @endif
            </div>
            ...
            <!-- Budget Items -->
            <div class="bg-white rounded-lg shadow-md p-4 mb-6">
                <h3 class="text-lg font-semibold mb-3">Budget Items</h3>
                    ...
                    <table class="w-full">
                        <thead>
                            <tr class="bg-gray-100 border-b border-gray-300">
                                ...
                                <th class="px-4 py-2 text-left text-xs font-semibold text-gray-700">Quantity</th>
                                <th class="px-4 py-2 text-left text-xs font-semibold text-gray-700">Unit Cost</th>
                                <th class="px-4 py-2 text-left text-xs font-semibold text-gray-700">Total</th>
                            </tr>
                        </thead>
                        <tbody>
                                ...
                                <tr class="{{ $index % 2 === 0 ? 'bg-white' : 'bg-gray-200' }} hover:bg-blue-200 transition-colors duration-150">
                                    ...
                                    <td class="px-4 py-2">
                                        ...
                                        @if($item->justification)
                                            <div class="text-xs text-gray-400 italic mt-1">{{ Str::limit($item->justification, 40) }}</div>
                                        @endif
                                    </td>
                                    <td class="px-4 py-2 whitespace-nowrap"><div class="text-xs text-gray-900">{{ $item->category ? ucfirst($item->category) : 'N/A' }}</div></td>
                                    <td class="px-4 py-2 whitespace-nowrap"><div class="text-xs text-gray-900">{{ $item->quantity }} {{ $item->unit }}</div></td>
                                    <td class="px-4 py-2 whitespace-nowrap"><div class="text-xs text-gray-900">${{ number_format($item->unit_cost, 2) }}</div></td>
                                    <td class="px-4 py-2 whitespace-nowrap"><div class="text-xs font-semibold text-gray-900">${{ number_format($item->total_cost, 2) }}</div></td>
                                </tr>
                            @endforeach
                            <tr class="bg-blue-50 font-bold border-t-2 border-blue-200">
                                <td colspan="5" class="px-4 py-2 text-right text-sm">Grand Total:</td>
                                <td class="px-4 py-2 text-base text-blue-600">${{ number_format($totalBudget, 2) }}</td>
                            </tr>
                        </tbody>
                    </table>
                </div>
            ...
            <!-- Approval Status -->
            <div class="bg-white rounded-lg shadow-md p-4 mb-6">
                <h3 class="text-lg font-semibold mb-3">Approval Status</h3>
            ...
            @if($userRole)
                <div class="bg-white rounded-lg shadow-md p-4 mb-6">
                    <h3 class="text-lg font-semibold mb-3">Your Approval Decision</h3>
